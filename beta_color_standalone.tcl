...
if { $frame >= $xnfr($molid) } { 
  error "Extended data not loaded for frame $frame"
}
if { $nat != $xnat($molid)} {
  error "Atoms number in molecule and in extended properties mismatch."
}
...
for {set iframe 0}  {$iframe< $nframes} { incr iframe } {
  ...
  if { $line != $nat } {
    error "Format error in reading XYZ* file"
  }
  set line [ gets $myfile ]
